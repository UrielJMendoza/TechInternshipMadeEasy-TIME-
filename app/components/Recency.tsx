...
function displayRecencyLabel(
  ...
  const isLegacy = /^(?:Posted|Found)\s+/i.test(label);
  if ((recalculate || isLegacy) && timestamp) {
    const currentLabel = relativeAge(timestamp);
    if (currentLabel) return currentLabel;
  }

  if (!isLegacy) return label;

  const compact = label.replace(/^(?:Posted|Found)\s+/i, "");
...
export function Recency({ kind, label, timestamp, recalculate = false }: RecencyProps) {
  const displayLabel = displayRecencyLabel(label, timestamp, recalculate);
  const semanticKind = kind === "posted" ? "Posted" : "Found";
  ...
  return (
    <strong
      ...
      aria-label={`${semanticKind} ${displayLabel.toLowerCase()}`}
    >
      <time dateTime={dateTime}>{displayLabel}</time>
    </strong>
  );
}
