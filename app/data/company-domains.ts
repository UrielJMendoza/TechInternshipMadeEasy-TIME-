...
export const COMPANY_DOMAINS: Readonly<Record<string, string>> = {
  aesop: "aesop.com",
  stripe: "stripe.com",
  figma: "figma.com",
  linear: "linear.app",
...
/** Guess a domain when the name isn't curated: drop suffix words, join, .com. */
...
export function companyDomain(company: string): string {
  ...
  const withoutSuffix = key.replace(
    /\s+(?:incorporated|inc|llc|ltd|limited|corp|corporation)$/,
    "",
  );
  return COMPANY_DOMAINS[key] ?? COMPANY_DOMAINS[withoutSuffix] ?? guess(company);
}
...
export function knownCompanyDomain(company: string): string | null {
  ...
  return COMPANY_DOMAINS[key] ?? COMPANY_DOMAINS[withoutSuffix] ?? null;
}

export function hasKnownCompanyDomain(company: string): boolean {
  return knownCompanyDomain(company) !== null;
}
