...
- `/companies` and `/companies/[slug]`: retired routes that permanently redirect to the job board (with a company search when a legacy slug is present).
...
- `/methodology`, `/status`, and `/companies`: retired public pages that redirect cleanly to `/jobs`; none appears in navigation, metadata, internal links, or the sitemap.
...
- `/robots.txt` allows public pages, disallows `/api/`, and advertises `/sitemap.xml`. The sitemap always includes useful static pages and adds only complete-snapshot, adequately populated collection pages plus legitimate active UUID job pages.
...
- Primary navigation now contains Home, Find Jobs, Tracker, Alerts, Account, and one Browse Jobs action. Companies, Discover duplication, How It Works, Methodology, and Data Status navigation were removed from desktop, mobile, and footer surfaces.
...
Company directory and detail pages are retired. Company-logo and job-detail company links now open a filtered `/jobs` view, and legacy company URLs permanently redirect there. Existing `timley:followed-companies:v1` browser data remains disclosed and clearable for users who created it before retirement.
...
- The global header contains Home, Find Jobs, Tracker, Alerts, Account, and one Browse Jobs action. Its native mobile disclosure supports keyboard use, Escape-to-close, focus return, and close-on-navigation.
...
- Growth coverage verifies copy/native share wiring, legacy company-follow clear-data coverage, data-backed discovery and campus routes, retired company redirects, fresh-role digest links, the changelog, and the active-listing-driven company marquee.
