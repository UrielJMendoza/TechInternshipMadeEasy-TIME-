...
import { SOURCE_IDS, SOURCE_REGISTRY } from "./sourceRegistry.ts";
...
async function failRun(
  ...
  const { error: recordError } = await serviceSupabase().rpc("fail_ingest_run", {
    run_id: runId,
...
export async function runIngest(
  ...
  try {
    const settled = await Promise.all(
      SOURCE_IDS.map(async (source) => {
        const sourceStartedAt = Date.now();
        try {
          const snapshot = await ADAPTERS[source]();
          const accepted = applyPostFilters(snapshot.jobs);
          const snapshotHash = await checksum(
            accepted.map(jobPayload).map((job) => JSON.stringify(job)).sort(),
          );
          return {
            ...
            value: {
              ...
              accepted,
              snapshotHash,
              duration: Date.now() - sourceStartedAt,
            },
          };
        } catch (reason) {
          return {
    ...
    settled.forEach((result, index) => {
      ...
      if (result.status === "rejected") {
        sourceResults.push({
          ...
          parser_version: SOURCE_REGISTRY[source].parser_version,
          snapshot_checksum: "",
      ...
      const { snapshot, accepted, snapshotHash, duration } = result.value;
      ...
      sourceResults.push({
        ...
        error: snapshot.health.issues.length
          ? snapshot.health.issues.slice(0, 10).map((issue) => issue.code).join(",")
          : null,
      });
    });
    ...
    const payload = acceptedJobs.map(jobPayload);
    const snapshotChecksum = await checksum(
      payload
    ...
    return {
      ...
      afterDedupe: dedupeJobs(acceptedJobs).length,
    ...
    await failRun(runId, sourceResults, error, startedAt);
    throw error;
  }
}
